...
const (
	masterAddress = "172.28.104.85:50060"               // Address of the master node
	clientAddress = "172.28.104.85:12345" // Address of the client server
)
...
func main() {
	...
	md := metadata.Pairs("client-ip", "172.28.104.85", "client-port", "12345")
	...
	for {
		...
		case "upload":
			...
			dataNodeAddress := fmt.Sprintf("%s%d", handleUploadFileResponse.IpAddress, handleUploadFileResponse.PortNumber)
			fmt.Println("Data Node Address for uploading:", dataNodeAddress)
		...
		case "download":
			...
			fmt.Println("Data Node Address for downloading:", dataNodeAddress)
			dataNodeConn, err := grpc.Dial(dataNodeAddress, grpc.WithInsecure(), grpc.WithDefaultCallOptions(grpc.MaxCallRecvMsgSize(1024*1024*100)))
			if err != nil {
				log.Fatalf("did not connect to data node: %v", err)
			}
